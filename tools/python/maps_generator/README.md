...
`maps_generator` is the Python CLI for generating `.mwm` maps for the Oma application. This tool functions as the driver for the `generator_tool` C++ executable.
...
The app version can be found in the "About" section of Oma app.
...
If you are not from the Oma team, then you do not need the `--production` option.
