...
To build and run SAARTHIyou'll need a machine with at least 4Gb of RAM and 20-30Gb of disk space depending on your target platform. Expect to download 5-10Gb of files.
...
First of all get the source code. The full SAARTHIsources repository is ~8.5Gb in size, there are various [clone options](#special-cases-options) to reduce the download size to suit your needs.
...
We haven't compiled SAARTHIon Windows *natively* in a long time, though it is possible.
...
SAARTHIhas some "hidden" debug commands that you can trigger by entering them into the search box.
...
Linux, MacOS, or Windows should work to build SAARTHIfor Android.
...
SAARTHIicon will appear in the application list in DHU.
...
Building SAARTHIfor iOS requires a Mac.
...
Enroll in the [Apple Developer Program](https://developer.apple.com/programs/) (you can run SAARTHIin Simulator without this step).
...
If you want to run SAARTHIon a real device, you have to remove the CarPlay entitlement. Open `iphone/Maps/OMaps-Debug.entitlements`
