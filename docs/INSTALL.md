...
To build and run Sarathi you'll need a machine with at least 4Gb of RAM and 20-30Gb of disk space depending on your target platform. Expect to download 5-10Gb of files.
...
First of all get the source code. The full Sarathi sources repository is ~8.5Gb in size, there are various [clone options](#special-cases-options) to reduce the download size to suit your needs.
...
You need a Linux or a MacOS machine to build a desktop version of Sarathi. [Windows](#windows) users can use the [WSL](https://learn.microsoft.com/en-us/windows/wsl/) (Windows Subsystem for Linux) and follow ["Linux or Mac"](#linux-or-mac) steps described below.
...
We haven't compiled Sarathi on Windows *natively* in a long time, though it is possible.
...
Sarathi has some "hidden" debug commands that you can trigger by entering them into the search box.
...
Linux, MacOS, or Windows should work to build Sarathi for Android.
...
Sarathi icon will appear in the application list in DHU.
...
Building Sarathi for iOS requires a Mac.
...
Enroll in the [Apple Developer Program](https://developer.apple.com/programs/) (you can run Sarathi in Simulator without this step).
...
If you want to run Sarathi on a real device, you have to remove the CarPlay entitlement. Open `iphone/Maps/OMaps-Debug.entitlements`
